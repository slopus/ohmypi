...
function waitForTerminalOutput(gym: Gym, marker: string, timeoutMs: number): Promise<void> {
    return new Promise((resolvePromise, reject) => {
        ...
        const timer = setTimeout(() => {
            stop();
            reject(new Error(`Timed out waiting for terminal output ${JSON.stringify(marker)}.`));
        }, timeoutMs);
    });
}
