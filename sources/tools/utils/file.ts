...
function planTextEditInContent(
  ...
  return {
    path: filePath,
    nextContent:
      content.slice(0, match.start) + nextString + content.slice(match.end),
...
function selectOccurrenceByLine(
  ...
  return positions.find((position) => lineNumberAtOffset(content, position) === options.lineNumber);
}
...
function replaceStraightQuotes(value: string, quote: string, pair: readonly [string, string]): string {
  let opening = true;
  let out = "";
...
export async function editTextFileBatch(
  ...
  for (let i = sorted.length - 1; i >= 0; i--) {
    ...
    const nextString = preserveQuoteStyle(match.edit.oldText, actualOldString, match.edit.newText);
