...
export class NativeProxessManager {
  ...
  async killAll(options: ProcessKillOptions = {}): Promise<void> {
    await Promise.all(
      [...this.#processes.values()].map((process) =>
        process.kill("SIGTERM", options),
      ),
    );
  }
}
...
export class ManagedProcess {
  ...
  constructor(options: ProcessStartOptions, onDone: (id: string) => void) {
    ...
    this.#cleanupProcessGroupOnExit =
      options.cleanupProcessGroupOnExit ?? false;
    this.#onDone = onDone;
  ...
  #maybeFinalizeAfterExit(): void {
    if (
      this.#settled ||
      (this.#exitCode === null && this.#exitSignal === null)
    ) {
      return;
    }
