...
export function createCodexProfileSummary(options: {
    ...
    return [
        ...
        `- Version note: the older compiled-client prompt hash comes from Codex CLI 0.144.6 (\`${compiledClientPromptSha256}\`), while the current tool capture uses CLI ${capture.tools.version}. Rig intentionally follows the newer source golden; the gym pins the older client hash separately.`,
        `- ${ultraNote}`,
        "",
